...
class GaussianModel(nn.Module):
    ...
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation, self.data_device)
            actual_covariance = L @ L.transpose(1, 2)
            symm = strip_symmetric(actual_covariance, self.data_device)
            return symm
    ...
    def __init__(self, args : ModelParams):
        super(GaussianModel, self).__init__()  # Ensure this is the first call
    ...
    def render(self, viewpoint_cam, pipe, background, render_func):
        return render_func(viewpoint_cam, self, pipe, background)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().to(self.data_device)
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().to(self.data_device))
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().to(self.data_device)
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0

        print("Number of points at initialisation : ", fused_point_cloud.shape[0])
        print(fused_point_cloud.device)

        dist2 = torch.clamp_min(distCUDA2(fused_point_cloud), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device=self.data_device)
        rots[:, 0] = 1

        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device=self.data_device))
        ...
        print('===========PCD CREATED RANK========', self.data_device, '===================')     
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        rots = build_rotation(r=self._rotation[selected_pts_mask], data_device=self.data_device).repeat(N,1,1)
